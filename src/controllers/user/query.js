...
const UserModal = {
    ...
    updateScore: async (score, userId) => {
        try {
         ...
         `
            return await client.query(updatedQry)
        }
        catch (error) {
            throw error;
        }
    },
    getUserBySfid: async (userSfid) => {
        try {
            const userQry = `
                SELECT ${OBJECTKEYNAME.PROFILE__C} FROM ${SCHEMA.SALESFORCE.USER} WHERE ${OBJECTKEYNAME.SFID} = '${userSfid}'
            `;
