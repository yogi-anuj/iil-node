...
exports.addRetailerMappingDetails=async(req,res)=>{
    try{
        console.log(API_END_POINT.ADD_RETAILER_MAPPING_DETAILS, '  ========> START');
        ...
        const {
          ...
          const checkRes = await retailerMapping.checkQuery(Mobile__c);
          if (checkRes.rows.length > 0 && checkRes.rows[0][OBJECTKEYNAME.MOBILE__C]) {
          ...
          const values = [
            ...
            console.log("test");
            const result = Retailer.insertRetailerDetails(values);
    console.log(API_END_POINT.ADD_RETAILER_MAPPING_DETAILS, '  ========> DATA');
    //await pool.query('COMMIT');
  //  await insertLeaderBoard(SCORE.MAPPING.RETAILER.SCORE,SCORE.MAPPING.RETAILER.NAME,currentUser.attributes.sfid)
...
exports.getRetailers=async(req,res)=>{
    ...
    console.log(API_END_POINT.GET_RETAILERS, '  ========> DATA');
    //await pool.query('COMMIT');
...
exports.getRetailerMappingDetails=async(req,res)=>{
    ...
    return res.json(
      responseBody(
        ...
        API_END_POINT.GET_RETAILER_MAPPING_DETAILS,
        false,
        response.rows || []
      )
    );
...
exports.updateRetailerMappingDetailsById=async(req,res)=>{
  try{
    ...
    const getQryResponse = Account.getAccountDetailById(herokuId);
