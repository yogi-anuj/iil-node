  ...
  getAllRetailersByTerritory: async (
    ...
    try {
      ...
      if (profile === USER_HIERARCHY.VP_user) {
        ...
        LEFT JOIN ${SCHEMA.SALESFORCE.VILLAGE__C} ON ${SCHEMA.SALESFORCE.VILLAGE__C}.${OBJECTKEYNAME.SFID} = ${SCHEMA.SALESFORCE.ACCOUNT}.${OBJECTKEYNAME.VILLAGE__C}
                `;
      } else if (profile === USER_HIERARCHY.AM__user || profile === USER_HIERARCHY.Crop_Advisor_user) {
        qry += `
  ...
  getRetailerById: async (herokuId) => {
    try {
      const getQry = `
        ...
        LEFT JOIN ${SCHEMA.SALESFORCE.ACCOUNT} as preferredDistributor ON ${SCHEMA.SALESFORCE.ACCOUNT}.${OBJECTKEYNAME.PREFERRED_DISTRIBUTOR__C} = preferredDistributor.${OBJECTKEYNAME.SFID}
        LEFT JOIN ${SCHEMA.SALESFORCE.ACCOUNT} as secondPreferredDistributor ON ${SCHEMA.SALESFORCE.ACCOUNT}.${OBJECTKEYNAME.SECOND_PREFERRED_DISTRIBUTOR__C} = secondPreferredDistributor.${OBJECTKEYNAME.SFID}
